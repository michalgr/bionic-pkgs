...
usage() {
  cat << 'EOF'
Usage: adb-push.sh [OPTIONS] --archive <ARCHIVE_PATH>

Options:
  -a, --archive <PATH>      Path to prebuilt runtime archive (.tar.gz) (Required)
  -n, --pkg-name <NAME>     Logical package name (e.g. rizin, strace, python3)
  -t, --target <TARGET>     Target architecture (default: aarch64-android)
...
if [ -z "$PKG_NAME" ]; then
  PKG_NAME="$(basename "$ARCHIVE_PATH" | sed -E 's/^[a-z0-9]{32}-//; s/\.tar\.(gz|zst)$//; s/\.tar$//; s/-(aarch64|x86_64|armv7a|i686)-unknown-linux-android//; s/-(aarch64|x86_64|armv7a|i686)-android//')"
fi
