...
let
  isPlatformPkg =
    p:
    let
      name = p.name or (p.pname or "");
    in
    lib.hasInfix "bionic" name;
  ...
  closeRuntimeClosure =
    ...
    let
      step =
        ...
        in
        if builtins.length next == builtins.length current then current else step next;
    in
    step pkgs;
  ...
  libcxxPkgs = lib.optional (
    needsLibcxx && stdenv.cc ? libcxx && stdenv.cc.libcxx != null
  ) stdenv.cc.libcxx;

  allPackages = lib.filter (p: !isPlatformPkg p) (lib.unique (basePackages ++ libcxxPkgs));

  getRuntimeOutputs =
    pkg:
    if lib.isDerivation pkg then
      let
        available = lib.filter (out: pkg ? ${out}) [
          "out"
          "bin"
          "lib"
        ];
