...
- **`lldb`**: Version check for `lldb` and `lldb-server`, batch process execution/exit status verification, target create and loaded image list inspection, breakpoint setting and control flow continuation.
