...
# 16. jq JSON processing and Oniguruma regex matching in sysroot
...
assert_contains "$output" "ok" "jq object property extraction in sysroot"

output="$(adb_shell "${ENV_WRAPPER} echo '[\"apple\", \"banana\", \"cherry\"]' | ${SYSROOT_DIR}/bin/jq 'map(test(\"^b\")) | any' 2>&1" || true)"
assert_contains "$output" "true" "jq regex matching in sysroot"
