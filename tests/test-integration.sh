...
ENV_SH="${SYSROOT_DIR}/env.sh"

# 1. strace tracing python3
output="$(adb_shell "${ENV_SH} strace -e trace=openat,write python3 -c \"import sys; print('strace+python integration ok')\" 2>&1" || true)"
...
"
output="$(adb_shell "${ENV_SH} python3 -c \"${bcc_py_code}\" 2>&1" || true)"
...
set +e
output="$(timeout 10 "$ADB_CMD" ${SERIAL:+-s $SERIAL} shell "${ENV_SH} bpftrace -e 'tracepoint:raw_syscalls:sys_enter { @[comm] = count(); } interval:s:1 { exit(); }' 2>&1")"
ret=$?
set -e
...
output="$(adb_shell "for b in ${SYSROOT_DIR}/bin/*; do [ -f \"\$b\" ] && [ -x \"\$b\" ] || continue; if head -c 4 \"\$b\" 2>/dev/null | grep -q 'ELF'; then ${ENV_SH} eu-readelf -h \"\$b\" >/dev/null 2>&1 || exit 1; fi; done; echo ALL_BINARIES_VALID_ELF 2>&1" || true)"
...
output="$(adb_shell "${ENV_SH} radare2 -q -c 'aaa; afl' ${SYSROOT_DIR}/bin/strace 2>&1" || true)"
assert_match "entry|main|sym" "$output" "radare2 disassembling sysroot binary (${SYSROOT_DIR}/bin/strace)"

output="$(adb_shell "${ENV_SH} rizin -q -c 'aa; afl' ${SYSROOT_DIR}/bin/strace 2>&1" || true)"
...
output="$(adb_shell "${ENV_SH} lldb --batch -o \"script import lldb; target = lldb.debugger.CreateTarget('${SYSROOT_DIR}/bin/strace'); print('LLDB_API_OK:', target.IsValid())\" -o \"quit\" 2>&1" || true)"
assert_contains "$output" "LLDB_API_OK: True" "lldb embedded python module and target API in sysroot"

output="$(adb_shell "${ENV_SH} lldb --batch -o \"target create ${SYSROOT_DIR}/bin/strace\" -o \"image list\" -o \"quit\" 2>&1" || true)"
...
output="$(adb_shell "${ENV_SH} gdb --batch -ex 'file ${SYSROOT_DIR}/bin/strace' -ex 'info files' -ex 'quit' 2>&1" || true)"
assert_contains "$output" "strace" "gdb target inspection of sysroot binary"

output="$(adb_shell "${ENV_SH} gdb --batch -ex 'python import gdb; print(\"GDB_INTEGRATION_OK:\", gdb.VERSION)' -ex 'quit' 2>&1" || true)"
...
output="$(adb_shell "echo 'socat_sysroot_integration' | ${ENV_SH} socat - - 2>&1" || true)"
...
adb_shell "nohup ${ENV_SH} iperf3 -s -1 -p 5210 > /data/local/tmp/iperf3-int-srv.log 2>&1 &"
sleep 1

output="$(adb_shell "${ENV_SH} iperf3 -c 127.0.0.1 -p 5210 -t 1 2>&1" || true)"
