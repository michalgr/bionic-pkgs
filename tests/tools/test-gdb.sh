...
if [ "$BIN_NAME" = "run.sh" ]; then
  ...
  GDB_CMD="PYTHONHOME='${BASE_DIR}' PYTHONPATH='${BASE_DIR}/share/gdb/python:${BASE_DIR}/lib/python3.13/site-packages' '${GDB_BIN}'"
fi

log_info "Testing gdb via: ${GDB_BIN}"
log_info "Testing gdbserver via: ${GDBSERVER_BIN}"

# 1. Version check for gdb CLI
output="$(adb_shell "${GDB_CMD} --version 2>&1" || true)"
assert_contains "$output" "GNU gdb" "gdb version check (--version)"

# 2. Version check for companion gdbserver
...
assert_match "/system/bin/sh" "$output" "gdb target ELF inspection"
...
assert_match "libc.so|Shared library|From" "$output" "gdb shared library loading"

# 7. Remote debugging
output="$(adb_shell "${GDBSERVER_BIN} --once 127.0.0.1:12345 /system/bin/echo server-test & sleep 1 && ${GDB_CMD} --batch -ex 'target remote 127.0.0.1:12345' -ex 'continue' -ex 'quit' 2>&1" || true)"
assert_match "Remote debugging|server-test|exited" "$output" "gdbserver remote debugging"

# 8. Python API evaluation
output="$(adb_shell "${GDB_CMD} --batch -ex 'python import gdb; print(\"GDB_PY_VERSION:\", gdb.VERSION)' -ex 'quit' 2>&1" || true)"
assert_contains "$output" "GDB_PY_VERSION:" "gdb python API evaluation"
